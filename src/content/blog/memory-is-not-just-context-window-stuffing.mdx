...
An agent repeats a tool call that already succeeded. The problem may look like reasoning, but the failure happened earlier: the system gave the model a transcript instead of authoritative execution state.

Other memory failures are quieter. A stale fact outranks a current one. A summary drops the exception that matters. A critic sees the draft but not the evidence. A private policy leaks into the wrong role.

Longer context does not solve these problems. It gives the system a larger space in which to mismanage them.

Prompt engineering asks how to phrase an instruction. Context engineering asks what should be in working memory for this step—and what must stay outside it.
...
## Six memory failures hiding inside agent failures
...
```mermaid
flowchart TB
  subgraph Prepare[Prepare the working set]
    direction LR
  ...
  subgraph Execute[Run a verified step]
    direction LR
    F[Compose context packet] --> G[Model call]
    G --> H[Validate output]
    H --> I[Commit durable state]
    I --> J[Compact, archive, or forget]
  end
  E --> F
  J --> B
```
...
## Separate what the agent knows from what the runtime knows
...
## Give each role the smallest useful view
...
## Use a memory hierarchy, not a larger bucket
...
## Different techniques manage different tiers
...
## Compression succeeds only when signal survives
...
## Personal agents need runtime memory
...
context_packet:
  task:
    ...
    run_id: "lifecycle-email-2026-05-04"
...
## Rules for trustworthy working memory
...
## Context is a runtime discipline

The next step is not merely a better model or a longer context window. It is an agent system that manages working memory with the discipline of an operating system:
...
When that discipline is explicit, smaller models can behave more reliably, simple agents can avoid expensive mistakes, and multi-agent systems can coordinate without reading the same noisy transcript.

Context engineering is not prompt polish. It is the runtime discipline of deciding what the model is allowed to treat as its working world.
