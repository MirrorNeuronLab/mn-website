...
An agent can have an excellent prompt and still be unable to answer a basic operational question: did the last action actually happen?

That question appears as soon as the demo meets time, tools, and failure. The agent must wait for data, survive a restart, avoid duplicate side effects, preserve an approval, coordinate a handoff, or stop at a policy boundary.

At that point, another clever instruction is not enough. The system needs a model of how work is owned, advanced, observed, and recovered.

Smart components need an operating model.

## The unit of design is the execution loop
...
## A prompt cannot own durable truth
...
## Give the workflow an operating model
...
## Five jobs that should not live in prompts

<Diagram
  title="The operating model surrounds every model call"
  description="Intelligence works inside state, boundaries, recovery, coordination, and observability."
  source={`flowchart TB
    A[State: what is true] --> F[Model or tool step]
    B[Boundaries: what is allowed] --> F
    C[Recovery: what can resume] --> F
    D[Coordination: who owns next] --> F
    E[Observability: what can be explained] --> F
    F --> G[Validated transition]
    G --> A`}
  caption="The model chooses within the workflow; the runtime preserves the workflow."
/>
...
## Judge the system by the completed work
...
## Simplicity matters most before the platform team
...
## Runtime quality compounds

Any AI infrastructure company whose moat is “we call the latest model” deserves skepticism.
...
## From prompting strategy to execution system
