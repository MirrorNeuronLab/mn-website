...
A support agent gives the correct refund answer—and calls the refund tool before checking whether the customer is eligible.

The sentence is right. The workflow is wrong.

As agents move from answering to acting, correctness expands beyond the final artifact. The path matters: which evidence was used, which tool was selected, which policy gate ran, which state was committed, and whether a retry repeated an action.

Verification is how those conditions become part of the system rather than suggestions in a prompt.

## Correctness exists at several layers
...
<Diagram
  title="Verify before the workflow changes the world"
  description="Checks happen at the boundary of each transition, not only after the final answer."
  source={`flowchart LR
    A[Propose step] --> B{Path allowed?}
    B -->|No| H[Block or escalate]
    B -->|Yes| C[Call model or tool]
    C --> D{Output valid?}
    D -->|No| I[Repair or retry]
    I --> C
    D -->|Yes| E{Policy satisfied?}
    E -->|No| H
    E -->|Yes| F[Commit state]
    F --> G[Advance]`}
  caption="The model proposes; the runtime decides what becomes durable state."
/>

## A correct answer can take an incorrect path
...
## A scorecard turns trust into evidence
...
## Tool calls are where language becomes consequence
...
## Put verification inside the execution loop
...
## Recovery depends on verified truth
...
## Correct outcome, correct path

An agent can produce a good sentence. A workflow has to produce the right outcome through an allowed, recoverable, and inspectable path.

That requires more than a final quality check. It requires output contracts, tool checks, state invariants, policy gates, and recovery validation at the moments where execution advances.

Verification is how capable models become software people can trust.
